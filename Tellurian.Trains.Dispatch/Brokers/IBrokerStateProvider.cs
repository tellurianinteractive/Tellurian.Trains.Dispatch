...
/// - Only one operation <see cref="SaveTrainsectionsAsync(IEnumerable{TrainSection}, CancellationToken)"/> or
///   <see cref="ReadTrainSections(CancellationToken)"/> at a time.
/// - If a <see cref="SaveTrainsectionsAsync(IEnumerable{TrainSection}, CancellationToken)"/> already is ongoing when
///   this function is called again, it should return immediately without saving.
/// </remarks>
public interface IBrokerStateProvider
{
    Task<bool> SaveTrainsectionsAsync(IEnumerable<TrainSection> trainSections, CancellationToken cancellationToken = default);
